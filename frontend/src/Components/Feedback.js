...
const Feedback = () => {
  ...
  const sendSubmit = async () => {
    ...
    if (!session_token) {
      setAfterSubmitMsg(
        "We appreciate you writing in a feedback. However, we ask that you register for an account first before writing in. Thank you."
      );
      return;
    }
    const data = await submitFeedback(session_token, feedbackMsg, followUp);
